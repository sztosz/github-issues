defmodule TableViewTest do
  ...
  def split_with_three_columns,
    do: TV.split_into_columns(simple_test_data, headers)

  test "split_into_three_columns" do
  ...
  test "column_widths" do
    widths = TV.widths_of(split_with_three_columns)
    assert widths == [ 5, 6, 7 ]
  end

  test "correct format string returned" do
    assert TV.format_for([9, 10, 11]) == "~-9s | ~-10s | ~-11s~n"
  end

  test "Output is correct" do
    result = capture_io fn ->
      TV.print_table_for_columns(simple_test_data, headers)
    end
  assert result == """
  c1    | c2     | c4     
  ------+--------+--------
